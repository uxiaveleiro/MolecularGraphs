...
test_dataset = dataset[:1880]
train_dataset = dataset[1880:]
...
class GCN(torch.nn.Module):
    def __init__(self, hidden_channels):
        ...
        self.conv1 = TransformerConv(dataset[0].num_node_features, hidden_channels, edge_dim=edge_dim, dropout=0.2, heads=nheads, concat=True) # ollo ao "hardcoded"
        self.conv2 = TransformerConv(hidden_channels*2, hidden_channels, edge_dim=edge_dim, dropout=0.2, heads=nheads, concat=True)
        ...
        self.lin = Linear(hidden_channels*2, 19) # harcoded now # *2 if concat True
...
model.to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=0.001) # decrease to 0.001
...
for epoch in range(1, max_epochs):
    ...
    if epoch%10==0:
        ...
        plt.savefig('test_results.png', dpi=330, bbox_inches='tight', pad_inches = 0.25)
